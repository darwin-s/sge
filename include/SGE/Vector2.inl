...
template<typename T> Vector2<T> Vector2<T>::operator*(T value) const {
...
template<typename T> Vector2<T>& Vector2<T>::operator*=(T value) {
    x = x * value;
    y = y * value;
    return *this;
}
...
template<typename T> T Vector2<T>::getMagnitude() const {
